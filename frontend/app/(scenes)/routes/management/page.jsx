...
export default function Management() {
  ...
  const saveRoute = async () => {
    ...
    const data = await apiFetch({
      url: id ? `route/${id}` : `routes/register`,
      method: id ? 'PUT' : 'POST',
      data: body
    })
  ...
  const routeEdition = async (id) => {
    ...
    const location = getBoundsForPoints(await data.route.places)
    ...
    setWaypoints(data.route.places)
    setSelectedOption(data.route.type)
    setCount(data.route.places.length)
    getRoute(data.route.type, data.route.places, false)
  }
        ...
        >
          {routeCreated ? (
            <>
              ...
              >
                <Wrapper width="50%">
                  <Button fluid onClick={() => saveRoute()}>
                    Salvar rota
                  </Button>
                </Wrapper>
                <Wrapper width="50%">
